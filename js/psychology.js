...
function renderPsychologyPage(){
  ...
  state.rules.forEach(r=>{
    ...
    <div class="rule-row" style="border-color:${rViolations>2?'rgba(255,51,102,0.2)':'var(--border)'};">
      ...
      <div style="flex:1;">
        <div style="font-size:12px;color:var(--text);line-height:1.4;">${r.text}</div>
...
function setDisciplineFilter(filter){
  ...
  const violCountEl = document.getElementById('disc-viol-count');
  if(violCountEl) violCountEl.textContent=violationCount()+' total violations';
  const psychViolEl = document.getElementById('psych-violations');
  if(psychViolEl){
    psychViolEl.textContent=violationCount();
    psychViolEl.style.color=violationCount()>8?'var(--red)':violationCount()>4?'var(--amber)':'var(--green)';
  }
  const revenge=state.trades.filter(t=>(t.emotion==='Greedy'||t.emotion==='Frustrated')&&t.outcome==='LOSS').length;
  const revengeEl = document.getElementById('psych-revenge');
  if(revengeEl) revengeEl.textContent=revenge;
  ...
  el.innerHTML='';
  state.rules.forEach(r=>{
    const col=r.violations===0?'var(--green)':r.violations<=2?'var(--amber)':'var(--red)';
    const bgLo=r.violations===0?'var(--green-lo)':r.violations<=2?'var(--amber-lo)':'var(--red-lo)';
    el.innerHTML+=`
    <div class="rule-row" style="border-color:${r.violations>2?'rgba(255,51,102,0.2)':'var(--border)'};">
      <div class="rule-icon" style="background:${bgLo};border:1px solid ${col}33;">
        ${r.violations===0?'✓':'⚠'}
      </div>
      <div style="flex:1;">
      ...
      <div class="rule-viol-count" style="color:${col};">${r.violations}</div>
    </div>`;
  });
  const dot=document.getElementById('psych-dot');
  if(dot){dot.style.display=violationCount()>5?'block':'none';}
}